...
def resunet_plus_plus(num_channnel:int=1, num_classes:int=7, height:int=48, width:int=48):  

    model = ResUnetPlusPlus(channel=num_channnel)
    return nn.Sequential(
        model,
        nn.AdaptiveAvgPool2d((height, width)),
        nn.Flatten(),
        nn.Linear(height*width, 4096),
        nn.ReLU(True),
        nn.Dropout(),
        nn.Linear(4096, 4096),
        nn.ReLU(True),
